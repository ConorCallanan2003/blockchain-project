<script lang="ts">
	...
	// var web3 = new Web3('wss://ethereum-sepolia-rpc.publicnode.com	');
	...
	const onClick = async () => {
		...
		var transaction = contract.methods.transfer(recipientAddress, quantity);
...
<div class="flex flex-col px-8 py-16 w-full h-[700px] justify-start items-center">
	<Dialog bind:dialog on:close={() => console.log('closed')}>
		<div class="flex p-4 flex-col justify-between">
			...
			<h2>You have sent {quantity} tokens!</h2>
	...
	<h1 class="text-5xl font-bold pb-10">Send Ticket</h1>
	<div
		class="w-[600px] h-[300px] p-8 flex flex-col gap-4 border-lg border-2 border-gray-200 rounded-2xl"
	>
		<div class="flex flex-col">
			<label class="text-lg font-medium" for="recipient-address">Recipient Address</label>
			<input
		...
		{#if loading}
			<button
				on:click={onClick}
				class="px-4 py-2 animate-loading-button w-full h-[80px] text-2xl text-white font-bold rounded-lg bg-gray-200 hover:bg-gray-300 hover:scale-105 active:scale-125 bg-[linear-gradient(to_right,theme(colors.indigo.400),theme(colors.sky.400),theme(colors.fuchsia.400),theme(colors.sky.400),theme(colors.indigo.400))] bg-[length:200%_auto] animate-gradient"
			>
				Sending
			</button>
		{:else}
			<button
				on:click={onClick}
				class="px-4 py-2 text-gray-800 w-full h-[80px] text-2xl font-bold rounded-lg bg-gray-200 hover:bg-gray-300 hover:scale-105 duration-200"
			>
				Send
			</button>
		{/if}
