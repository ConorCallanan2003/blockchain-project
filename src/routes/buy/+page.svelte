<script lang="ts">
	...
	const onClick = async () => {
		...
		var transaction = contract.methods.buy(quantity);
...
<div class="flex flex-col px-8 py-16 w-full h-[700px] justify-start items-center">
	<Dialog bind:dialog on:close={() => console.log('closed')}>
		<div class="flex p-4 flex-col justify-between">
			...
			<h2>You have bought {quantity} tokens!</h2>
	...
	<h1 class="text-5xl font-bold pb-10">Buy Ticket(s)</h1>
	<div
	...
	>
		...
		{#if quantity > 0}
			<p class="text-xl font-medium">
				Cost: {web3.utils.fromWei(tokenCost * quantity, 'ether')} ETH
			</p>
		{/if}
		{#if loading}
			<button
